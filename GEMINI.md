...
- **Hierarchical Loading and Precedence:** The CLI implements a sophisticated hierarchical memory system by loading context files (e.g., `GEMINI.md`) from several locations. Content from files lower in this list (more specific) typically overrides or supplements content from files higher up (more general). The exact concatenation order and final context can be inspected using the `/memory show` command. The typical loading order is:
  ...
  2.  **Project Root & Ancestors Context Files:**
      - Location: The CLI searches for the configured context file in the current working directory and then in each parent directory up to either the git repository root or the user's home directory.
      - Scope: Provides project-specific context, with files closer to your current directory taking higher precedence.
  3.  **Include Directories Context Files (if configured):**
      - Location: Context files from directories specified in `includeDirectories` setting (controlled by `loadMemoryFromIncludeDirectories` setting).
      - Scope: Additional project context from related directories.

### Context File Management Commands

The CLI provides several commands to help you manage and understand your context:

- **`/memory show`**: Displays the full context that will be provided to the model, including all loaded context files and their sources.
- **`/memory refresh`**: Reloads all context files from disk, useful when you've made changes to context files during a session.
- **`/memory clear`**: Temporarily clears the loaded context for the current session (context will be reloaded on next session).

### Best Practices for Context Files

1. **Keep it Relevant:** Include information that's directly useful for the tasks you'll be performing with the AI.
2. **Be Specific:** Provide concrete examples and guidelines rather than vague descriptions.
3. **Update Regularly:** Keep your context files current with your project's evolution.
4. **Use Hierarchy:** Place general guidelines in higher-level context files and specific details in project-specific files.
5. **Test and Iterate:** Use `/memory show` to verify your context is loading correctly and refine based on the AI's responses.

## Usage Statistics

Gemini CLI can collect anonymous usage statistics to help improve the tool. This data includes information about command usage, error rates, and performance metrics, but does not include sensitive information like your code, prompts, or personal data.

- **Control:** You can enable or disable usage statistics collection using the `usageStatisticsEnabled` setting in your `settings.json` file.
- **Privacy:** All collected data is anonymized and aggregated. No personally identifiable information or code content is collected.
- **Transparency:** The CLI will inform you when usage statistics are being collected and provide options to opt out.

## Sandboxing

Sandboxing provides an additional layer of security when using Gemini CLI by isolating tool execution from your main system. This is particularly useful when working with untrusted code or when you want to limit the potential impact of AI-generated commands.

### Types of Sandboxing

1. **Docker Sandboxing** (`"sandbox": "docker"`):
   - Uses Docker containers to isolate tool execution
   - Requires Docker to be installed and running
   - Provides strong isolation from the host system

2. **Podman Sandboxing** (`"sandbox": "podman"`):
   - Similar to Docker but uses Podman as the container runtime
   - Useful in environments where Docker is not available

3. **Custom Sandboxing** (`"sandbox": "custom-command"`):
   - Allows you to specify a custom sandboxing command
   - Provides flexibility for specialized environments

### Configuration

You can enable sandboxing through:
- Settings file: `"sandbox": true` or `"sandbox": "docker"`
- Environment variable: `GEMINI_SANDBOX=true`
- Command line: `--sandbox` flag

### Custom Sandbox Profiles

For advanced users, you can create custom sandbox profiles by placing configuration files in your project's `.gemini/` directory:

- **Docker:** `.gemini/sandbox.Dockerfile`
- **macOS Seatbelt:** `.gemini/sandbox-macos-<profile_name>.sb`

These custom profiles allow you to define specific security policies and runtime environments for your project's needs.